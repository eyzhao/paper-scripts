...
match_full = re.compile(r'\{\?var:.*?\}')
match_key = re.compile(r'\{\?var:(.*?)\}')


def prepare(doc):
    doc.variables = yaml.load(
        open(doc.get_metadata('variables', default='variables.yml'))
    )
    doc.boldface_variables = doc.get_metadata(
        'boldface_variables',
        default=False
    )
